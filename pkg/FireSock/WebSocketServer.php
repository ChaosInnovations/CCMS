...
use \Package\Logger;
use \Package\FireSock\WebSocketUser;
...
class WebSocketServer
{
    ...
    public function __construct($address, $port, $bufferLength=2048)
    {
        ...
        if (!socket_set_option($this->masterSocket, SOL_SOCKET, SO_REUSEADDR, 1)) {
            $this->logger->log("Failed to start server: socket_option()", "Initialize");
            return;
        }

        if (!socket_bind($this->masterSocket, $address, $port)) {
            $this->logger->log("Failed to start server: socket_bind()", "Initialize");
            return;
        }

        if (!socket_listen($this->masterSocket, 20)) {
            $this->logger->log("Failed to start server: socket_listen()", "Initialize");
            return;
        }

        $this->sockets['m'] = $this->masterSocket;

        $this->logger->log("Server running", "Initialize");
        $this->logger->log("Listening on {$address}:{$port}", "Initialize");
        ...
        while (true) {
            ...
            foreach ($r as $socket) {
                if ($socket == $this->masterSocket) {
                    ...
                    if ($client < 0) {
                        $this->logger->log("Failed to accept connection", "Connection");
                        continue;
                    }

                    $this->connect($client);
                    $this->logger->log("Client connected: " . $client, "Connection");
                    continue;
                }
                ...
                if ($numBytes === false) {
                    ...
                    switch ($sockErrNo)
                    {
                        ...
                        case 125: // ECANCELED    -- Operation canceled
                            $this->logger->log("Unusual disconnect on socket " . $socket . ": " . socket_strerror($sockErrNo), "Connection");
                            $this->disconnect($socket, true, $sockErrNo);
                            break;
                        default:
                            $this->logger->log("Socket error: " . socket_strerror($sockErrNo), "Connection");
                    }
                    continue;
                }

                if ($numBytes == 0) {
                    $this0>disconnect($socket);
                    $this->logger->log("Client disconnected. TCP connectionlost: " . $socket);
                    continue;
                }
    ...
    private function process($user, $message)
    {
        $user->processMessage($this, $message);
    }

    private function connected($user)
    {

    }

    private function closed($user)
    {
    ...
    private function disconnect($socket, $triggerClosed=true, $sockErrNo=null)
    {
        ...
        if ($disconnectedUser !== null) {
          ...
          if ($triggerClosed) {
            $this->logger->log("Client disconnected. ".$disconnectedUser->socket, "Connection");
    ...
    private function splitPacket($length, $packet, $user)
    {
