...
use \Package\SecureMenu\PanelPage;
use \Package\User;
use \PDO;
    ...
    public static function hookUpdate(Request $request)
    {
            ...
            if (substr($_POST["chat"], 0, 1) == "R") {
                ...
                if ($room["room_members"] == "*") {
                    ...
                    foreach($stmt->fetchAll() as $u) {
                        (new User($u["uid"]))->notify(User::$currentUser, "R" . $rid);
                    }
                } else {
                    $members = explode(";", $room["room_members"]);
                    foreach($members as $m) {
                        (new User($m))->notify(User::$currentUser, "R" . $rid);
                    }
                }
            } else {
                $uid = substr($_POST["chat"], 1);
                (new User($uid))->notify(User::$currentUser, "U" . User::$currentUser->uid);
            }
        }
        ...
        if (isset($_POST["entry"])) {
            ...
            if ($list["list_participants"] == "*") {
                ...
                $stmt->execute();$stmt->setFetchMode(PDO::FETCH_ASSOC);
                foreach($stmt->fetchAll() as $u) {
                    (new User($u["uid"]))->notify(User::$currentUser, "L" . $_POST["list"]);
                }
            } else {
                $members = explode(";", $list["list_participants"]);
                foreach($members as $m) {
                    (new User($m))->notify(User::$currentUser, "L" . $_POST["list"]);
                }
            }
        }
